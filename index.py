...
class PasswordHandler(webapp.RequestHandler):
	def post(self):
		...
		if str(hashlib.sha1(password).hexdigest()) == '3fcfd14762bd3087e8563832e048945e50df7f80': # matching password, access data
			doRender(self, 'menu.htm')
		else:
			doRender(self, 'login.htm',
				{'message': 'Incorrect Password'})

class BulkSubmitHandler(webapp.RequestHandler):
	def get(self):
		doRender(self, 'bulkSubmitData.htm')

class ManualSubmitHandler(webapp.RequestHandler):
	def get(self):
		memberNames = ['Cory Derringer', 'Kelsey Derringer', 'Henry McKay', 'Sarah McKay', 
		'Jim Marino', 'Bill Hathaway', 'Bigfoot Sasquatch', 'Katrina Loutzenhiser', 'Jess Rice', 
		'Tom Marvolo Riddle', 'Bill Nye', 'Joe Dart', 'Commander Meouch', 'Count Chocula']
		doRender(self, 'manualSubmitData.htm', {
			'memberNames': memberNames
			})

class QueryHandler(webapp.RequestHandler):
	def get(self):
		doRender(self, 'queryData.htm')
...
# 			que=db.Query(ScenarioData)
# 			que.filter("usernum >", 240).order("usernum").order("scenario").order("trialNumber")
# 			d=que.fetch(limit=10000)
...
application = webapp.WSGIApplication([
	...
	('/bulkData', BulkSubmitHandler),
	('/manualData', ManualSubmitHandler),
